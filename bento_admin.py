...
def split_bento_compartments(img_bgr: np.ndarray, bento_mask: np.ndarray) -> List[np.ndarray]:
    ...
    compartment_masks = []
    min_area = img_bgr.shape[0] * img_bgr.shape[1] * 0.01 # 判定基準を1%に緩和
    ...
    # もしエリアが1つも見つからなかった場合、容器全体を1つのエリアとする
...
def calc_detailed_ratios(img_bgr, comp_masks, food_mask):
    ...
    results = []
    # 視認性の高い色セット
    colors = [(0, 255, 0), (255, 255, 0), (255, 0, 0), (0, 255, 255), (255, 0, 255)]
    ...
    for i, m in enumerate(comp_masks):
        ...
        cv2.drawContours(annotated_rgb, cnts, -1, color, 10)
        ...
        if M['m00'] != 0:
            ...
            # 縁取り付きテキスト
            cv2.putText(annotated_rgb, txt, (cx-60, cy), cv2.FONT_HERSHEY_BOLD, 3.0, (0,0,0), 15)
            cv2.putText(annotated_rgb, txt, (cx-60, cy), cv2.FONT_HERSHEY_BOLD, 3.0, (255,255,255), 5)
...
if uploads:
    ...
    with cr:
        idx = sel.get("selection", {}).get("rows", [0])[0]
        if not df.empty:
